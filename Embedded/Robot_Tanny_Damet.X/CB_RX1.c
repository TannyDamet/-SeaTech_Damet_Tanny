...
void CB_RX1_Add(unsigned char value) {
    if (CB_RX1_GetRemainingSize() > 0) {
        cbRx1Buffer[cbRx1Head] = value; // Ajoute la valeur à la position pointée par le pointeur de tête
        cbRx1Head++;
...
int CB_RX1_GetRemainingSize(void) {
    ...
    int remainingSize = CBRX1_BUFFER_SIZE - CB_RX1_GetDataSize(); // Calcul la taille restante disponible en soustrayant la taille des données de la taille totale du tampon
